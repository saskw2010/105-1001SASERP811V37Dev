    ...
    Public Class ReportBase
        ...
        Sub IHttpHandler_ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
            ...
            If Not (String.IsNullOrEmpty(aa)) Then
                ...
                result = controller.Execute(args.Controller, args.View, args)
            ...
            If templateName = "TestSession" Then
                ' i need to go with table
                AppendDownloadTokenCookie()


                Dim pagereport1 As ViewPage = ControllerFactory.CreateDataController().GetPage(request.Controller, "grid1", request)
                Dim tablerequestreport1 As DataTable = pagereport1.ToDataTable()
                ...
                Dim reportFileNamemossopath As String = appDirectory1 + "\Reportsdata\"
                ...
                For Each file In New DirectoryInfo(reportFileNamemossopath).GetFiles(myfilestring)
                    file.Delete()
                Next
                ...
                tablerequestreport1.WriteXml(xmlWrite, System.Data.XmlWriteMode.IgnoreSchema)
                tablerequestreport1.WriteXmlSchema(reportFileNamemossopath + reportFileNamemosso + ".xsd")

                result.NavigateUrl = "~/flashbased/test/qa.aspx"
                If Not (String.IsNullOrEmpty(result.NavigateUrl)) Then
                    AppendDownloadTokenCookie()
                    result.NavigateUrl = result.NavigateUrl
                    context.Response.Redirect(result.NavigateUrl)
                End If


            End If
            If templateName = "_blank" Then
                ...
                result.NavigateUrl = "~/pages/webreportviwerDyn1.aspx?reportquery=" & Right(args.Path.ToString(), Len(args.Path.ToString()) - 4) & "&controllername=" & args.Controller & "&viewname=grid1" & "&sosdesc=" + request.FilterDetails + "&reportFileNamemosso=" + reportFileNamemosso
                If Not (String.IsNullOrEmpty(result.NavigateUrl)) Then
                    AppendDownloadTokenCookie()
                    result.NavigateUrl = result.NavigateUrl
                    context.Response.Redirect(result.NavigateUrl)
                    'ResponseHelper.Redirect1(result.NavigateUrl)
                End If


            End If
            If templateName = "mospdf" Then
                ' i need to go with table
                ' AppendDownloadTokenCookie()
                'Dim wokwoak As Double = Len("&templateName=" + templateName + "&c=" + c + "&aa=" + aa + "&q=" + q + "&sosdesc=" + request.FilterDetails)
                'result.NavigateUrl = result.NavigateUrl + "&lenth=" + wokwoak.ToString() + "&templateName=" + templateName + "&c=" + c + "&aa=" + aa + "&q=" + q + "&sosdesc=" + request.FilterDetails
                'context.Response.Redirect(result.NavigateUrl)
                Dim pagereport1 As ViewPage = ControllerFactory.CreateDataController().GetPage(request.Controller, "grid1", request)
                Dim tablerequestreport1 As DataTable = pagereport1.ToDataTable()
                ' context.Session("tablerequestreport") = tablerequestreport1
                ...
                result.NavigateUrl = "~/pages/webreportviwerDynpdf.aspx?reportquery=" & Right(args.Path.ToString(), Len(args.Path.ToString()) - 10) & "&controllername=" & args.Controller & "&viewname=grid1" & "&sosdesc=" + request.FilterDetails + "&reportFileNamemosso=" + reportFileNamemosso
                If Not (String.IsNullOrEmpty(result.NavigateUrl)) Then
                    AppendDownloadTokenCookie()
                    result.NavigateUrl = result.NavigateUrl
                    context.Response.Redirect(result.NavigateUrl)
            ...
            If templateName = "mosxlx" Then
                ...
                ' Dim reportFileName1 As String = appDirectory1 + "\Reportsdata\mostafaxlml" + username + timestampstring + ".xsd"
                tablerequestreport1.WriteXmlSchema(reportFileNamemossopath + reportFileNamemosso + ".xsd")
                result.NavigateUrl = "~/pages/webreportviwerDynxlx.aspx?reportquery=" & Right(args.Path.ToString(), Len(args.Path.ToString()) - 10) & "&controllername=" & args.Controller & "&viewname=grid1" & "&sosdesc=" + request.FilterDetails + "&reportFileNamemosso=" + reportFileNamemosso
                If Not (String.IsNullOrEmpty(result.NavigateUrl)) Then
                    AppendDownloadTokenCookie()
                    result.NavigateUrl = result.NavigateUrl
                    context.Response.Redirect(result.NavigateUrl)
            ...
            End If
            If templateName = "DaynamicMrtReport" Then

                Dim dpagereport As ViewPage = ControllerFactory.CreateDataController().GetPage(request.Controller, "grid1", request)
                Dim tablerequestreport As DataTable = dpagereport.ToDataTable()
                ...
                tablerequestreport.WriteXml(xmlWrite, System.Data.XmlWriteMode.IgnoreSchema)
                ' Dim reportFileName1 As String = appDirectory1 + "\Reportsdata\mostafaxlml" + username + timestampstring + ".xsd"


                'doc.Save(xmlWrite)
                tablerequestreport.WriteXmlSchema(reportFileNamemossopath + reportFileNamemosso + ".xsd")
                result.NavigateUrl = "~/pages/webreportviwerDynr.aspx?reportquery=" & Right(args.Path.ToString(), Len(args.Path.ToString()) - 4) & "&controllername=" & args.Controller & "&viewname=" & args.View & "&sosdesc=" + request.FilterDetails + "&reportFileNamemosso=" + reportFileNamemosso
                If Not (String.IsNullOrEmpty(result.NavigateUrl)) Then
                    AppendDownloadTokenCookie()
                    result.NavigateUrl = result.NavigateUrl
                    context.Response.Redirect(result.NavigateUrl)
