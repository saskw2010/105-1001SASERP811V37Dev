...
Namespace eZee.Rules
    ...
    Partial Public Class schvchdlydetailregBusinessRules
        ...
        ''' </summary>
        <Rule("withoutargr100")>  _
        Public Sub withoutargr100Implementation( _
                    ByVal trs_No As Nullable(Of Long),  _
                    ByVal schvchdlyhdregid As Nullable(Of Long),  _
                    ByVal schvchdlyhdregNotes As String,  _
            ...
            If ActionArgs.Current.CommandArgument = "Mony_Paid" Then
			HttpContext.Current.Session("schvchdlyhdregid") = ActionArgs.Current.Item("schvchdlyhdregid").NewValue
                sqlstatmentcheck = "select count(*) as tecko from schvchdlydetailreg  where  schvchdlyhdregid=" & ActionArgs.Current.Item("schvchdlyhdregid").NewValue & " and [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue & ""
                ...
                If tekost = 0 Then
                    ...
                    sqlstatment = "INSERT INTO [schvchdlydetailreg] ([schvchdlyhdregid]  ,[branch] ,[StudentCode]  ,[acdcode] ,[FatherCode],[Trsd_Dt],[schpaymenttypeid]" & _
                   ",[Mony_Paid] ,[Notes],[Sumofdebitcal] ,[Sumofcerditcal],[Sumofdebitcal1],[totalbalance1])" & _
                   " select " & ActionArgs.Current.Item("schvchdlyhdregid").NewValue & " as idnumberid ,[branch] ,[StudentCode] ,[acdcode],[FatherCode] ,Getdate(),[schpaymenttypeid],0 ,[FirstName1] ,[trdb0] ,[trcr1]" & _
                   " ,[trdb1] ,[Balance] FROM [GLschassmentBalance1001] where  [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue 
                    xinsert = dbinerto.Insert(sqlstatment)
                   
                        sqlstatmentcheck = "select count(*) as tecko from schvchdlydetailreg  where  schvchdlyhdregid=" & ActionArgs.Current.Item("schvchdlyhdregid").NewValue & " and [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue & ""
                        Dim tekostxcv As Object
                        tekostxcv = DataLogic.GetValue(sqlstatmentcheck)
                    If tekostxcv > 0 Then
                        Result.ExecuteOnClient(
                            "this.set_lastCommandName(null);" & _
                            "this.goToView('gridnew');" & _
                            "var dv=Web.DataView.find('dv101Extender');" & _
                            "if(dv)dv.refresh();")
 
			Context.Session("ControllerID") = Arguments.ContextKey
            'Result.ShowModal("MyEmployeeTerritories", "createTerritory","New", "createTerritory")
			Result.ShowModal("Cardstudent", "CreateForm1", "New", "CreateForm1")
			else
			Result.ShowAlert("The Student Havenot Balance ")
			Result.ExecuteOnClient(
                            "this.set_lastCommandName(null);" & _
                            "this.goToView('gridnew');" & _
                            "var dv=Web.DataView.find('dv101Extender');" & _
...
'"var dv=Web.DataView.showModal(null,'testcontroller','createForm1','New','createForm1', this.get_baseUrl(), this.get_servicePath(), [{{Name:'rosom', Value: {0}}}]);" +
'"dv._parentDataViewId=this.get_id();" +
'"dv._lastViewId=this.get_viewId();",15))
                    End If
            ...
            'totalbalance
            If ActionArgs.Current.CommandArgument = "totalbalance" Then
                Dim tekost1 As Object
                If IsNothing(ActionArgs.Current.Item("studentCode").NewValue) Or IsNothing(ActionArgs.Current.Item("acdcode").NewValue) Then
                Else
                    If IsNothing(ActionArgs.Current.Item("schpaymenttypeid").NewValue) Then
                    Else
                        sqlstatmentcheck = "select trdb1  FROM [GLschassmentBalance1001] where schpaymenttypeid=" & ActionArgs.Current.Item("schpaymenttypeid").NewValue & " and [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue & " and [acdcode]=" & ActionArgs.Current.Item("acdcode").NewValue
                        tekost1 = DataLogic.GetValue(sqlstatmentcheck)
                        If IsNothing(tekost1) Then

                        Else
                            UpdateFieldValue("Sumofdebitcal1", tekost1)


                        End If
                        sqlstatmentcheck = "select trcr1  FROM [GLschassmentBalance1001] where schpaymenttypeid=" & ActionArgs.Current.Item("schpaymenttypeid").NewValue & " and [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue & " and [acdcode]=" & ActionArgs.Current.Item("acdcode").NewValue
                        tekost1 = DataLogic.GetValue(sqlstatmentcheck)
                        If IsNothing(tekost1) Then

                        Else
                            UpdateFieldValue("Sumofcerditcal", tekost1)



                        End If
                        sqlstatmentcheck = "select Balance  FROM [GLschassmentBalance1001] where schpaymenttypeid=" & ActionArgs.Current.Item("schpaymenttypeid").NewValue & " and [StudentCode]=" & ActionArgs.Current.Item("studentCode").NewValue & " and [acdcode]=" & ActionArgs.Current.Item("acdcode").NewValue
                        tekost1 = DataLogic.GetValue(sqlstatmentcheck)
                        If IsNothing(tekost1) Then
                        Else

                            UpdateFieldValue("totalbalance1", tekost1)
